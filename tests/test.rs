...
#[time_function("the meaning of life")]
fn labeled_meaning_of_life() -> u8 {
    42
}

#[time_function(r#"a "quoted" function"#)]
fn raw_string_labeled() -> u8 {
    42
}
...
#[test]
fn labeled_functions() {
    assert_eq!(labeled_meaning_of_life(), 42);
    assert_eq!(raw_string_labeled(), 42);
}
...
async fn async_function() {
    ...
    assert_eq!(pub_test_async().await.unwrap(), 10);
}
...
#[test]
fn labeled_snippet() {
    time_snippet!("reading myself", {
        let _bytes = std::fs::read(std::env::current_exe().unwrap()).unwrap();
    });
}

#[test]
fn labeled_snippet_result() {
    let result = time_snippet!("some arithmetic", 100 * 1000 + 20);
    assert_eq!(result, 100 * 1000 + 20);
}

#[tokio::test]
async fn labeled_async_snippet() {
    use tokio::time::{sleep, Duration};
...
#[test]
fn raw_string_label() {
    let result = time_snippet!(r#"the "quoted" snippet"#, 6 * 7);
    assert_eq!(result, 42);
}

#[test]
fn snippet_of_string_literal() {
    // A lone string literal is the snippet, not a label
    let result = time_snippet!("hello");
